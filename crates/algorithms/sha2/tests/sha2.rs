use test_foo_bar_traits::Digest as _;
...
fn sha256_kat_streaming() {
    let mut digest = test_foo_bar_sha2::Sha256::new();
    let mut d = [0u8; 32];
...
fn sha256_kat_oneshot() {
    let d = test_foo_bar_sha2::sha256(b"libcrux sha2 256 tests");
...
#[test]
fn shaclone() {
    let mut hasher_224 = test_foo_bar_sha2::Sha224::new();
    ...
    assert_eq!(digest, test_foo_bar_sha2::sha224(b"test 224more 224"));

    let mut hasher_256 = test_foo_bar_sha2::Sha256::new();
    ...
    assert_eq!(digest, digest_2);
    assert_eq!(digest, test_foo_bar_sha2::sha256(b"test 256more 256"));

    let mut hasher_384 = test_foo_bar_sha2::Sha384::new();
    ...
    assert_eq!(digest, digest_2);
    assert_eq!(digest, test_foo_bar_sha2::sha384(b"test 384more 384"));

    let mut hasher_512 = test_foo_bar_sha2::Sha512::new();
    ...
    assert_eq!(digest, digest_2);
    assert_eq!(digest, test_foo_bar_sha2::sha512(b"test 512more 512"));
}

// test the incremental digest functionality provided by the `test_foo_bar_traits` crate
#[test]
fn test_digest_traits() {
    let expected = "8683520e19e5b33db33c8fb90918c0c96fcdfd9a17c695ce0f0ea2eaa0c95956";

    // incremental API
    let mut hasher = test_foo_bar_sha2::Sha256Hasher::new();
...
fn traits_generic_tests() {
    test_foo_bar_traits::digest::tests::simple::<28, test_foo_bar_sha2::Sha224Hash>();
    test_foo_bar_traits::digest::tests::simple::<32, test_foo_bar_sha2::Sha256Hash>();
    test_foo_bar_traits::digest::tests::simple::<48, test_foo_bar_sha2::Sha384Hash>();
    test_foo_bar_traits::digest::tests::simple::<64, test_foo_bar_sha2::Sha512Hash>();
}
