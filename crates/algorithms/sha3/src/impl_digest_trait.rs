...
macro_rules! impl_hash_traits {
    ($type:ident, $hasher:ident, $len:expr, $method:expr) => {
        ...
        pub type $hasher = test_foo_bar_traits::digest::Hasher<$len, $type>;

        impl test_foo_bar_traits::digest::arrayref::Hash<$len> for $type {
            #[inline(always)]
            fn hash(
                digest: &mut [u8; $len],
                payload: &[u8],
            ) -> Result<(), test_foo_bar_traits::digest::arrayref::HashError> {
                if payload.len() > u32::MAX as usize {
                    return Err(test_foo_bar_traits::digest::arrayref::HashError::InvalidPayloadLength);
                }
...
mod slice {
    ...
    test_foo_bar_traits::digest::slice::impl_hash_trait!(Sha3_224 => SHA3_224_LEN);
    test_foo_bar_traits::digest::slice::impl_hash_trait!(Sha3_256 => SHA3_256_LEN);
    test_foo_bar_traits::digest::slice::impl_hash_trait!(Sha3_384 => SHA3_384_LEN);
    test_foo_bar_traits::digest::slice::impl_hash_trait!(Sha3_512 => SHA3_512_LEN);
}
