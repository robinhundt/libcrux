use test_foo_bar_ecdh::{self, key_gen};
...
fn derive_rand() {
    ...
    let (private_a, public_a) = key_gen(test_foo_bar_ecdh::Algorithm::P256, &mut rng).unwrap();
    let (private_b, public_b) = key_gen(test_foo_bar_ecdh::Algorithm::P256, &mut rng).unwrap();

    let shared_a =
        test_foo_bar_ecdh::derive(test_foo_bar_ecdh::Algorithm::P256, &public_b, &private_a).unwrap();
    let shared_b =
        test_foo_bar_ecdh::derive(test_foo_bar_ecdh::Algorithm::P256, &public_a, &private_b).unwrap();
...
fn derive() {
    ...
    let public_a_comp =
        test_foo_bar_ecdh::secret_to_public(test_foo_bar_ecdh::Algorithm::P256, &private_a).unwrap();
    assert_eq!(public_a, public_a_comp);
    let public_b_comp =
        test_foo_bar_ecdh::secret_to_public(test_foo_bar_ecdh::Algorithm::P256, &private_b).unwrap();
