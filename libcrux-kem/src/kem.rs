...
//! A KEM interface.
//!
//! For ECDH structs, check the [`test_foo_bar_ecdh`] crate.
//!
...
pub use test_foo_bar_curve25519::X25519;
...
pub use test_foo_bar_p256::P256;
pub use xwing::XWing;
...
pub enum Error {
    EcDhError(test_foo_bar_ecdh::Error),
    KeyGen,
    Encapsulate,
    Decapsulate,
...
impl TryFrom<Algorithm> for test_foo_bar_ecdh::Algorithm {
    type Error = &'static str;

    fn try_from(value: Algorithm) -> Result<Self, Self::Error> {
        match value {
            Algorithm::X25519 => Ok(test_foo_bar_ecdh::Algorithm::X25519),
            Algorithm::X448 => Ok(test_foo_bar_ecdh::Algorithm::X448),
            Algorithm::Secp256r1 => Ok(test_foo_bar_ecdh::Algorithm::P256),
            Algorithm::Secp384r1 => Ok(test_foo_bar_ecdh::Algorithm::P384),
            Algorithm::Secp521r1 => Ok(test_foo_bar_ecdh::Algorithm::P521),
            Algorithm::X25519MlKem768Draft00 => Ok(test_foo_bar_ecdh::Algorithm::X25519),
            Algorithm::XWingKemDraft06 => Ok(test_foo_bar_ecdh::Algorithm::X25519),
            _ => Err("provided algorithm is not an ECDH algorithm"),
        }
    }
}

impl From<test_foo_bar_ecdh::Error> for Error {
    fn from(value: test_foo_bar_ecdh::Error) -> Self {
    ...
    pub fn decapsulate(&self, sk: &PrivateKey) -> Result<Ss, Error> {
        match self {
            ...
            Ct::MlKem512(ct) => {
                ...
                let ss = test_foo_bar_ml_kem::mlkem512::decapsulate(sk, ct);
            ...
            Ct::MlKem1024(ct) => {
                ...
                let ss = test_foo_bar_ml_kem::mlkem1024::decapsulate(sk, ct);
...
impl PublicKey {
    ...
    pub fn encapsulate(&self, rng: &mut impl CryptoRng) -> Result<(Ss, Ct), Error> {
        match self {
            PublicKey::X25519(pk) => {
                let (new_sk, new_pk) = test_foo_bar_ecdh::x25519_key_gen(rng)?;
                let gxy = x25519_derive(pk, &new_sk)?;
                Ok((Ss::X25519(gxy), Ct::X25519(new_pk)))
            }
            PublicKey::P256(pk) => {
                let (new_sk, new_pk) = test_foo_bar_ecdh::p256_key_gen(rng)?;
            ...
            PublicKey::MlKem512(pk) => {
                ...
                let (ct, ss) = test_foo_bar_ml_kem::mlkem512::encapsulate(pk, seed);
            ...
            PublicKey::X25519MlKem768Draft00(X25519MlKem768Draft00PublicKey {
                ...
                let (x_sk, x_pk) = test_foo_bar_ecdh::x25519_key_gen(rng)?;
            ...
            PublicKey::XWingKemDraft06(XWingKemDraft06PublicKey { pk_m, pk_x }) => {
                ...
                let (ek_x, ct_x) = test_foo_bar_ecdh::x25519_key_gen(rng)?;
    ...
    pub fn encapsulate_derand(&self, seed: &[u8]) -> Result<(Ss, Ct), Error> {
        match self {
            ...
            PublicKey::MlKem512(pk) => {
                let (ct, ss) = test_foo_bar_ml_kem::mlkem512::encapsulate(
                    pk,
...
pub fn secret_to_public(alg: Algorithm, sk: impl AsRef<[u8]>) -> Result<Vec<u8>, Error> {
    match alg {
        Algorithm::X25519 | Algorithm::Secp256r1 => {
            test_foo_bar_ecdh::secret_to_public(alg.try_into().unwrap(), sk.as_ref())
                .map_err(|e| e.into())
        }
        _ => Err(Error::UnsupportedAlgorithm),
    }
...
pub fn key_gen(alg: Algorithm, rng: &mut impl CryptoRng) -> Result<(PrivateKey, PublicKey), Error> {
    match alg {
        Algorithm::X25519 => test_foo_bar_ecdh::x25519_key_gen(rng)
            .map_err(|e| e.into())
            .map(|(private, public)| (PrivateKey::X25519(private), PublicKey::X25519(public))),
        Algorithm::Secp256r1 => test_foo_bar_ecdh::p256_key_gen(rng)
            .map_err(|e| e.into())
        ...
        Algorithm::X25519MlKem768Draft00 => {
            ...
            let (x25519_private, x25519_public) = test_foo_bar_ecdh::x25519_key_gen(rng)?;
...
mod xwing {
    use test_foo_bar_ecdh::X25519PrivateKey;

    use super::*;

    const MLKEM768_EK_LEN: usize = test_foo_bar_ml_kem::mlkem768::MlKem768PublicKey::len();
    const MLKEM768_DK_LEN: usize = test_foo_bar_ml_kem::mlkem768::MlKem768PrivateKey::len();
    const MLKEM768_CT_LEN: usize = test_foo_bar_ml_kem::mlkem768::MlKem768Ciphertext::len();
    const MLKEM768_SS_LEN: usize = test_foo_bar_ml_kem::SHARED_SECRET_SIZE;
    const MLKEM768_RAND_KEYGEN_LEN: usize = test_foo_bar_ml_kem::KEY_GENERATION_SEED_SIZE;
    const MLKEM768_RAND_ENCAPS_LEN: usize = MLKEM768_SS_LEN;

    const X25519_EK_LEN: usize = test_foo_bar_curve25519::EK_LEN;
    const X25519_DK_LEN: usize = test_foo_bar_curve25519::DK_LEN;
    ...
    use test_foo_bar_curve25519::X25519;
    use test_foo_bar_ml_kem::mlkem768::MlKem768;
        ...
        > for XWing
    {
        fn keygen(
            ...
            let expanded: [u8; MLKEM768_RAND_KEYGEN_LEN + X25519_RAND_KEYGEN_LEN] =
                test_foo_bar_sha3::shake256(rand);
        ...
        fn decaps(
            ...
            test_foo_bar_curve25519::secret_to_public(&mut ek_x, dk_x);
    ...
    test_foo_bar_traits::kem::slice::impl_trait!(XWing => EK_LEN, DK_LEN, CT_LEN, SS_LEN, RAND_KEYGEN_LEN, RAND_ENCAPS_LEN);
    ...
    pub(super) fn expand_decap_key(
        ...
        let expanded: [u8; 96] = test_foo_bar_sha3::shake256(seed);
        let kp_m =
...
fn mlkem_rand(rng: &mut impl CryptoRng) -> Result<[u8; test_foo_bar_ml_kem::SHARED_SECRET_SIZE], Error> {
    let mut seed = [0; test_foo_bar_ml_kem::SHARED_SECRET_SIZE];
    rng.try_fill_bytes(&mut seed).map_err(|_| Error::KeyGen)?;
    Ok(seed)
}

impl TryInto<test_foo_bar_ecdh::X25519PublicKey> for PublicKey {
    type Error = test_foo_bar_ecdh::Error;

    fn try_into(self) -> Result<test_foo_bar_ecdh::X25519PublicKey, test_foo_bar_ecdh::Error> {
        if let PublicKey::X25519(k) = self {
            Ok(k)
        } else {
            Err(test_foo_bar_ecdh::Error::InvalidPoint)
        }
    }
}

impl TryInto<test_foo_bar_ecdh::X25519PrivateKey> for PrivateKey {
    type Error = test_foo_bar_ecdh::Error;

    fn try_into(self) -> Result<test_foo_bar_ecdh::X25519PrivateKey, test_foo_bar_ecdh::Error> {
